	...
	this.display_items=function(){
	   ...
	   for(var k in this.items){
	     ...
	     /* html_content+="<td>";
	      ...
	      html_content+="</td>";
		  html_content+="<td>";
	      html_content+="<span onclick='mycart.deleteItem("+this.items[k].id+")' class='btn'>Delete</span>";
	      html_content+="</td>";
		  html_content+="<td>&pound";
	      html_content+=this.items[k].total_price;
	      html_content+="</td>";
	      html_content+="</tr>";
		  sum_price+=parseFloat(this.items[k].total_price);
	 }
